 ...
 * Reads CSV + live Firestore transactions only; nothing is written to reconciliations/*.
 *
...
function parseArgs(argv) {
  ...
  return { matchFrom, positional, writeReport, fullReport };
}
...
function formatReport({
  ...
  lines.push(``);
  lines.push(
    `This file is generated by \`functions/scripts/recon-match-dry-run.mjs\`. It does **not** create or update \`clients/*/reconciliations/*\`. It only reads your CSV and live \`transactions\` for comparison. Normalized bank amounts are **integer centavos**, same as SAMS.`
  );
  lines.push(``);
  ...
  lines.push(`| Parsed CSV rows (in window) | ${bankRowCount} (of ${parsedCount} total parsed) |`);
  ...
  for (const m of matches) {
    ...
    if (m.speiFeeGapCentavos != null) {
      lines.push(
        `  - Scotia SPEI fee+IVA gap (bank vs |SAMS|): ${m.speiFeeGapCentavos} centavos`
      );
    }
...
async function main() {
  const { matchFrom, positional, writeReport, fullReport } = parseArgs(process.argv);
  const csvPath =
    positional[0] ||
  ...
  const { bankRows: parsedRows, errors } = await parseScotiabankCSV(buf);
  if (errors.length) {
  ...
  console.log(
    `CSV: ${parsedRows.length} parsed rows → ${bankRows.length} rows from ${matchFrom} onward | span ${startDate} .. ${endDate}`
  );
  console.log(`Client: ${clientId} | account: ${accountId}\n`);

  const rawNorm = normalizeRowsForSession('scotiabank', bankRows);
  const normalizedRows = rawNorm.map((nr, i) => ({
    id: `dry-norm-${i}`,
    ...nr
  }));

  console.log(`Normalized (reference groups): ${normalizedRows.length} rows\n`);
