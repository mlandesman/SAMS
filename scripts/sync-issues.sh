...
function show_weekly_summary() {
    ...
    gh issue list --state closed --search "closed:>=$(date -d '7 days ago' +%Y-%m-%d)" --limit 10
    
    echo ""
    echo -e "${BLUE}Technical Debt Items:${NC}"
    grep "^### \*\*TD-" "$PROJECT_ROOT/TECHNICAL_DEBT.md" | wc -l | xargs echo "  Total tracked:"
    grep "Priority: High" "$PROJECT_ROOT/TECHNICAL_DEBT.md" | wc -l | xargs echo "  High priority:"
