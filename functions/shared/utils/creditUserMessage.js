...
/**
 * @param {string} text
 * @param {{ source?: string, type?: string }} context
 * @returns {boolean}
 */
export function isAutogeneratedUserMessage(text, { source, type }) {
  return String(text || '').trim() === getAutogeneratedUserMessage({ source, type }).trim();
}
 ...
 * @returns {boolean}
 */
export function isAutogeneratedUserMessageEs(text, { source, type }) {
  return String(text || '').trim() === getAutogeneratedUserMessageEs({ source, type }).trim();
}
...
export function resolveCreditUserMessageEs({ userMessage, userMessage_es, notes, source, type }) {
  ...
  const autogenEn = getAutogeneratedUserMessage({ source, type });
  if (resolvedEn.trim() === autogenEn.trim()) {
    return getAutogeneratedUserMessageEs({ source, type });
  }

  // Custom or legacy friendly English — do not substitute generic template Spanish
  return '';
}
