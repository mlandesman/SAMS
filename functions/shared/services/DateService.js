 ...
 * 
 * CRITICAL: All dates in SAMS must use America/Cancun timezone
 * CRITICAL: NEVER use new Date() - ALWAYS use getNow() from this service
 * 
 * Architecture:
 * - Fiscal Year: July 1 - June 30
 * - Timezone: America/Cancun (UTC-5, no DST)
 * - Month Numbering: 0-11 (July=0, June=11)
 * 
 * Reusability: 100% across Water Bills, HOA Dues, Reports, Propane Tanks
 * 
 * @module shared/services/DateService
 */
class DateService {
...
/**
 * CRITICAL: Standalone function to replace new Date() system-wide
 * Returns current time in Cancun timezone as a JavaScript Date object
 * 
 * USAGE: Always use getNow() instead of new Date() throughout SAMS
 * 
 * @returns {Date} JavaScript Date object representing current Cancun time
 * 
 * @example
 * // WRONG - Never do this
 * const now = new Date(); // ❌ Wrong timezone
 * 
 * // CORRECT - Always do this
 * import { getNow } from '@/shared/services/DateService.js';
 * const now = getNow(); // ✅ Correct Cancun timezone
 */
const getNow = () => {
  // Get current time in Cancun timezone
  const nowInCancun = DateTime.now().setZone('America/Cancun');
