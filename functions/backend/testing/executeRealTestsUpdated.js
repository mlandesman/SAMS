/**
 * Real Test Execution Using Test Harness - Updated for new error messages
...
// UPDATE EXPECTED ERROR MESSAGES TO MATCH BACKEND
const ERROR_MESSAGE_UPDATES = {
  ...
  'Token from invalid project': 'Token is from incorrect Firebase project',
...
function convertTestToHarnessFormat(testId, testSpec) {
  ...
  if (testSpec['Expected Result']?.Body?.error) {
    const originalError = testSpec['Expected Result'].Body.error;
    if (ERROR_MESSAGE_UPDATES[originalError]) {
      testSpec['Expected Result'].Body.error = ERROR_MESSAGE_UPDATES[originalError];
      console.log(`   📝 Updated expected error from "${originalError}" to "${ERROR_MESSAGE_UPDATES[originalError]}"`);
    }
  }
      ...
      async test() {
        ...
        try {
          ...
          const passed = response.status === expectedStatus && 
                        (!expectedBody || checkBodyMatch(response.data, expectedBody).matches);
        ...
        if (testId.startsWith('AUTH-') && headers.Authorization) {
          ...
          if (headers.Authorization.includes('${')) {
            ...
            headers.Authorization = headers.Authorization
              .replace('${EXPIRED_TOKEN}', 'eyJhbGciOiJSUzI1NiIsImtpZCI6ImV4cGlyZWQiLCJleHAiOjEwMDAwMDAwMDB9.eyJleHAiOjEwMDAwMDAwMDB9.expired')
              .replace('${DIFFERENT_PROJECT_TOKEN}', 'eyJhbGciOiJSUzI1NiIsImtpZCI6ImRpZmZlcmVudCIsInByb2plY3QiOiJvdGhlciJ9.eyJwcm9qZWN0IjoiZGlmZmVyZW50In0.signature')
...
async function main() {
  ...
  console.log('📝 Updated with new backend error messages');
