...
class CreditService {
  ...
  async updateCreditHistoryEntry(clientId, unitId, entryId, updates) {
    try {
      ...
      const messageSource = updatedEntry.source || oldSource;
      if (
        newType !== oldType
        && updates.userMessage === undefined
        && updates.userMessage_es === undefined
      ) {
        const oldAutoEn = getAutogeneratedUserMessage({ source: messageSource, type: oldType });
        const oldAutoEs = getAutogeneratedUserMessageEs({ source: messageSource, type: oldType });
        ...
        if (currentEn === oldAutoEn.trim()) {
          updatedEntry.userMessage = getAutogeneratedUserMessage({ source: messageSource, type: newType });
        }
        if (!currentEs || currentEs === oldAutoEs.trim()) {
          updatedEntry.userMessage_es = getAutogeneratedUserMessageEs({ source: messageSource, type: newType });
        }
      }
