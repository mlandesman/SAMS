  ...
  async updateCreditHistoryEntry(clientId, unitId, entryId, updates) {
    try {
      ...
      if (updates.date !== undefined) {
        const dt = DateTime.fromISO(String(updates.date), { zone: 'America/Cancun' });
        if (!dt.isValid) {
          throw new Error(`Invalid date format: ${updates.date}`);
        }
        updatedEntry.timestamp = dt.toUTC().toISO();
      }
      ...
      const newType = inferEntryType(updatedEntry);
      const newSource = updatedEntry.source || oldSource;
      const typeChanged = newType !== oldType;
      const sourceChanged = newSource !== oldSource;
      if (
        (typeChanged || sourceChanged)
        && updates.userMessage === undefined
        && updates.userMessage_es === undefined
      ) {
        const oldAutoEn = getAutogeneratedUserMessage({ source: oldSource, type: oldType });
        const oldAutoEs = getAutogeneratedUserMessageEs({ source: oldSource, type: oldType });
        const currentEn = String(updatedEntry.userMessage || '').trim();
        const currentEs = String(updatedEntry.userMessage_es || '').trim();

        if (currentEn === oldAutoEn.trim()) {
          updatedEntry.userMessage = getAutogeneratedUserMessage({ source: newSource, type: newType });
        }
        if (!currentEs || currentEs === oldAutoEs.trim()) {
          updatedEntry.userMessage_es = getAutogeneratedUserMessageEs({ source: newSource, type: newType });
        }
      }
