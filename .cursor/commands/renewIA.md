...
1. **Handover Document** Write everything we did so far to a properly formatted handover document using the instructions found in `/Users/michael/Projects/SAMS-Docs/apm/prompts/Implementation_Agent/Implementation_Agent_Handover_Guide.md`. Ensure to clearly document the approach we're taking, the steps we've done so far, and the current failure we're working on. 
