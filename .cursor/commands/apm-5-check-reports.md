...
1. Determine scan scope:
   - If `(the text provided by the User after the command invocation, if any)` provided, resolve each agent-id per `.cursor/skills/apm-communication/SKILL.md` §4.2 Agent ID Resolution. Batch-read all targeted Report Buses in a single terminal invocation, e.g., `cat .apm/bus/<slug-1>/report.md .apm/bus/<slug-2>/report.md` (or platform equivalent). Continue to step 3.
