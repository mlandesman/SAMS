...
2. Resolve agent-id (unregistered Workers only): resolve `(the text provided by the User after the command invocation, if any)` against `.apm/bus/` directory names per `.cursor/skills/apm-communication/SKILL.md` §4.2 Agent ID Resolution. Initialize per `.cursor/commands/apm-3-initiate-worker.md` §2 Initiation.
...
4. Cross-validate `agent` field in YAML frontmatter against registered identity. Mismatch flags a routing error - decline and direct User to the correct Worker. Process the Task per `.cursor/apm-guides/task-execution.md` §3 Task Execution Procedure.
