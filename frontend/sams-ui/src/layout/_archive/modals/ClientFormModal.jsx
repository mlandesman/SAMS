...
const ClientFormModal = ({ 
  ...
  useEffect(() => {
    if (mode === 'edit' && initialData) {
      // Handle both new template structure and legacy structure
      const isLegacyStructure = !initialData.basicInfo;
      
      setFormData({
        basicInfo: {
          fullName: isLegacyStructure ? 
            (initialData.fullName || initialData.name || '') : 
            (initialData.basicInfo?.fullName || ''),
          clientId: isLegacyStructure ? 
            (initialData.id || '') : 
            (initialData.basicInfo?.clientId || ''),
          displayName: isLegacyStructure ? 
            (initialData.displayName || initialData.shortName || '') : 
            (initialData.basicInfo?.displayName || ''),
          clientType: isLegacyStructure ? 
            (initialData.clientType || initialData.type || 'HOA_Management') : 
            (initialData.basicInfo?.clientType || 'HOA_Management'),
          status: isLegacyStructure ? 
            (initialData.status || 'active') : 
            (initialData.basicInfo?.status || 'active'),
          description: isLegacyStructure ? 
            (initialData.description || initialData.notes || '') : 
            (initialData.basicInfo?.description || '')
        },
        branding: {
          logoUrl: isLegacyStructure ? 
            (initialData.logoUrl || initialData.logo || null) : 
            (initialData.branding?.logoUrl || null),
          iconUrl: isLegacyStructure ? 
            (initialData.iconUrl || initialData.icon || null) : 
            (initialData.branding?.iconUrl || null),
          brandColors: {
            primary: isLegacyStructure ? 
              (initialData.themeColor || '#2563eb') : 
              (initialData.branding?.brandColors?.primary || '#2563eb'),
            secondary: isLegacyStructure ? 
              '#64748b' : 
              (initialData.branding?.brandColors?.secondary || '#64748b'),
            accent: isLegacyStructure ? 
              '#10b981' : 
              (initialData.branding?.brandColors?.accent || '#10b981')
          }
        },
        configuration: {
          timezone: 'America/Cancun', // Always use Cancun timezone
          currency: isLegacyStructure ? 
            'MXN' : 
            (initialData.configuration?.currency || 'MXN'),
          language: isLegacyStructure ? 
            'es-MX' : 
            (initialData.configuration?.language || 'es-MX'),
          dateFormat: isLegacyStructure ? 
            'DD/MM/YYYY' : 
            (initialData.configuration?.dateFormat || 'DD/MM/YYYY')
        },
        contactInfo: {
          primaryEmail: isLegacyStructure ? 
            (initialData.email || '') : 
            (initialData.contactInfo?.primaryEmail || ''),
          phone: isLegacyStructure ? 
            (initialData.phone || '') : 
            (initialData.contactInfo?.phone || ''),
          address: {
            street: isLegacyStructure ? 
              (initialData.address?.street || '') : 
              (initialData.contactInfo?.address?.street || ''),
            city: isLegacyStructure ? 
              (initialData.address?.city || '') : 
              (initialData.contactInfo?.address?.city || ''),
            state: isLegacyStructure ? 
              (initialData.address?.state || '') : 
              (initialData.contactInfo?.address?.state || ''),
            postalCode: isLegacyStructure ? 
              (initialData.address?.postalCode || '') : 
              (initialData.contactInfo?.address?.postalCode || ''),
            country: isLegacyStructure ? 
              (initialData.address?.country || 'MX') : 
              (initialData.contactInfo?.address?.country || 'MX')
          },
          website: isLegacyStructure ? 
            (initialData.website || '') : 
      ...
      // Set logo preview
      const logoUrl = isLegacyStructure ? 
        (initialData.logoUrl || initialData.logo) : 
        (initialData.branding?.logoUrl);
      if (logoUrl) {
        setLogoPreview(logoUrl);
      }
      
      // Set icon preview
      const iconUrl = isLegacyStructure ? 
        (initialData.iconUrl || initialData.icon) : 
        (initialData.branding?.iconUrl);
      if (iconUrl) {
