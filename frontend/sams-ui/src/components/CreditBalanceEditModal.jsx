...
function CreditBalanceEditModal({ isOpen, onClose, unitId, currentBalance, year, onUpdate, mode }) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setNotes('');
      const initialEn = getAutogeneratedUserMessage({ source: 'admin', type: 'credit_added' });
      const initialEs = getAutogeneratedUserMessageEs({ source: 'admin', type: 'credit_added' });
      setUserMessage(initialEn);
      setUserMessageEs(initialEs);
      userMessageRef.current = initialEn;
      setSource('admin');
  ...
  useEffect(() => {
    ...
    const autoEnForNew = getAutogeneratedUserMessage({ source: previewSource, type: previewType });
    const autoEsForNew = getAutogeneratedUserMessageEs({ source: previewSource, type: previewType });

    if (!userMessageTouchedRef.current) {
      setUserMessage(autoEnForNew);
      userMessageRef.current = autoEnForNew;
    } else {
      setUserMessage((prev) => {
        const next = prev.trim() === autoEnForPrev.trim() ? autoEnForNew : prev;
        userMessageRef.current = next;
        return next;
      });
    }
    ...
    if (!userMessageEsTouchedRef.current) {
      if (!userMessageTouchedRef.current) {
        setUserMessageEs(autoEsForNew);
      } else {
        const currentEn = userMessageRef.current.trim();
        const effectiveEn = currentEn === autoEnForPrev.trim() ? autoEnForNew : currentEn;
        setUserMessageEs(deriveSpanishCompanionForEnglishInput(effectiveEn));
      }
    } else {
      setUserMessageEs((prev) => (
        prev.trim() === autoEsForPrev.trim() ? autoEsForNew : prev
      ));
    }
