...
function TransactionDetails({ transaction, onEdit, onDelete }) {
  ...
  const formatDate = (date) => {
    ...
    try {
      ...
      let dateObj;
      if (date.timestampValue) {
        // Handle nested timestampValue from API
        dateObj = new Date(date.timestampValue);
      } else if (date.toDate) {
        dateObj = date.toDate();
    ...
    <div className="transaction-details">
      ...
      <div className="transaction-details-content">
        <div className="transaction-field">
          ...
          <div className="field-value">{formatDate(transaction.date?.timestampValue || transaction.date)}</div>
        </div>
        ...
        <div className="transaction-field">
          ...
          <div className="field-value">{transaction.vendorName?.stringValue || transaction.vendorName || transaction.vendor || 'N/A'}</div>
        </div>
        ...
        <div className="transaction-field">
          ...
          <div className="field-value">{transaction.categoryName?.stringValue || transaction.categoryName || transaction.category || 'N/A'}</div>
        </div>
        ...
        <div className="transaction-field">
          ...
          <div className={`field-value ${(transaction.amount?.integerValue || transaction.amount) < 0 ? 'negative-amount' : 'positive-amount'}`}>
