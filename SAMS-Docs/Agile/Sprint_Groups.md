...
|---|-------|----------|------------|
| **308** | [Bug] Allocation of deposit funds using credit balance does not clear the Water Bills | **blocker** | ✅ Closed Apr 22, 2026 — data-only reconciliation |

**Outcome**: Salmon-colored herring. Looked like a UPC writer-side cache-corruption defect; turned out to be a small import-era data miss amplified by a missing invariant between two intentionally-independent credit derivations.

**Root Cause (final)**: The December 2025 import migration from the prior Google-Sheet-based system used text-parsing scripts to synthesize credit-history entries. The parsing was good but not perfect — it under-recorded a handful of overpayment events on a small number of units (centavos to ~$100 per affected unit). Those silent under-credits sat dormant for months because no payment had depended on the credit ledger to fully cover a bill. The April 20 unit 202 payment was the first to hit one of the affected units; the credit ledger had only $1.16 of credit when the SoA had been correctly displaying $3.48; the UPC allocator drew the $1.16 it had, leaving Water Q3 ~$2.18 short — which the writer faithfully persisted as `status: "partial"`.

**No defect in runtime code**: UPC writer, allocator, SoA derivation, and the credit-ledger utilities all operated correctly given their inputs. The original Bootstrap's "writer-side cache corruption" hypothesis and the Bootstrap Addendum's "single-class import miss on unit 202" hypothesis were both proved wrong during the diagnostic loop.

**Phase Outcomes**:
- **Phase A — Diagnose**: Replaced by interactive PO/Manager dialog before any IA was assigned. Diagnosis converged on the SoA-vs-ledger divergence pattern and explicitly excluded the writer/allocator/SoA code paths as defect sites. See `Manager_Bootstrap_Addendum_Sprint_UPC_CREDIT_FIX_2026-04-21.md`.
- **Phase B — Minimal Fix**: Cancelled. No code change required.
- **Phase C — Audit + Reconciliation**: IA built read-only audit script (`auditCreditLedgerVsSoA.js`) which flagged 10/20 units. Eight were by-design false positives (SoA visual application of credit against future-window charges, working as designed per PO clarification on PH4D and PH1A). Two real defects: AVII 202 (+$0.14 residual from PO's manual Water Q3 fix) and AVII 103 (+$99.88 cumulative drift across three import-era misattributions on Oct 2025–Jan 2026 transactions). PO posted both reconciliation entries directly via the Credit Balance UI's "+ Add Credit → Edit Source" workflow in both Dev and Prod. Audit script was deleted at sprint close (false-positive rate too high without #311's enhancement); branch deleted (no merge to main).

**Status**: ✅ COMPLETE (April 22, 2026) — no runtime code changed; two reconciliation entries posted via UI; #308 closed; sprint archived.

**Reference Issue**: https://github.com/mlandesman/SAMS/issues/308
...
### 🌙 Sprint DATA-INTEGRITY-1: Nightly Data Integrity Validator + Ops Summary Email (Deferred)
*Defense-in-depth data drift detection + consolidated morning operations report*

| # | Title | Priority | Est |
|---|-------|----------|-----|
| **312** | [ENHANCEMENT] Nightly Data Integrity Validation + Operations Summary Email | medium | 8-12h (framework + 1 check + email rollup) |

**Theme**: Build a pluggable nightly Firebase-scheduled validator that runs cross-reference data integrity checks against production, persists findings, surfaces critical issues via the existing System Errors dashboard infrastructure, and emits a single consolidated morning "honey-do list" email consolidating data integrity findings + backup status + exchange rate download status + overnight critical errors.

**Sprint Intent**: Detect data drift from any source (manual UI edits via creditBalances CRUD, future code regressions, partial batch failures, backup round-trip anomalies) within ≤24 hours, and replace scattered nightly notifications with a single morning summary.

**Deliverables**:
- **A**: Validator framework + initial check (UPC `getCreditBalance` vs SoA-derived credit balance parity).
- **B**: Consolidated nightly operations summary email replacing standalone backup/FX failure emails.

**Architectural Alignment**: Per SAMS Accounting & Payment Architecture §"Core Principle 3" the validator is strictly read-only. Per §"Handling Discrepancies" findings are surfaced for operator action; auto-correction is forbidden. See `SAMS-Docs/SAMS Guides/INDEX_Accounting_Payments_Billing.md` Pattern Catalog.

**Risk**: Low (read-only, scheduled job, no UI changes). Maintenance discipline is the ongoing concern — validator checks must be reviewed for staleness when data model changes.

**Dependencies**: None. Independent of all current sprints.

**Status**: Deferred (filed Apr 22, 2026 — defer until PO ready to schedule).

**Non-Goals (NON-NEGOTIABLE)**:
- ❌ NO auto-correction of any finding
- ❌ NO real-time validation (nightly only)
- ❌ NO user-facing config UI in v1 (Firestore-document or env config)
- ❌ NO scope expansion beyond the framework + 1 check + email rollup in v1
...
**Archive**: `SAMS-Docs/Sprint_Management/Sprint_Archive/Sprint_MOBILE_TX_UX_2026-03-29/`
...
**Archive**: `SAMS-Docs/Sprint_Management/Sprint_Archive/WhatsApp_Webhook_WA-BACKEND_2026-03-19/` (historical completion log for PR #253 merge; **operational “done” not claimed**)
...
**Archive**: `SAMS-Docs/Sprint_Management/Sprint_Archive/WhatsApp_Webhook_WA-BACKEND_2026-03-19/`
...
**Quality Rating**: ⭐⭐⭐⭐⭐  
**Archive**: `SAMS-Docs/Sprint_Management/Sprint_Archive/Sprint_EM_System_Error_Monitor_2026-02-16/`
